...
inline __device__ uint64_t hashtable_lookup(HashLookupParam param, uint64_t key) {
    int bucket_num = do_1st_hash(key, param.num_buckets);
    for (int i = 0; i < param.num_funcs; ++i) {
        int pos = bucket_num * BUCKET_SIZE +
                  do_2nd_hash(key, param.config, i, BUCKET_SIZE);
        if (param.d_key[i * param.size + pos] == key) {
            return param.d_val[i * param.size + pos] + 1;
        }
    }
    return EMPTY_CELL;
}
