...
class BaseModel(pl.LightningModule):
    ...
    def log_geometry(self, name: str, geom, draw_color: bool = False):
        ...
        if isinstance(geom, o3d.geometry.TriangleMesh):
            ...
            render_kwargs = {
                'geoms': [geom],
                'up_axis': '+Y',
                'viewport_shading': 'LIT' if draw_color else 'NORMAL'
            }

            try:
                mv_img = render.multiview_image(**render_kwargs, backend=self.render_backend)
            except Exception:
                # --privileged allows us to use VSCode inside docker, but will let pyrender fail :(
                self.render_backend = 'filament'
                mv_img = render.multiview_image(**render_kwargs, backend=self.render_backend)
    ...
    def on_test_start(self):
        if self.hparams.get('record', None) is not None:
            ...
            self.record_folder = Path("records") / test_set_name / save_folder_name
